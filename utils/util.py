...
def quatify_mc_and_net_change(parsed_result, mc_range=(0, 1e9), 
                              net_in_1min_range=(0, 1e9), net_in_5min_range=(0, 1e9), net_in_diff_range=(0, 1e9)):
    '''量化市值和净流入的变化
    ...
    return markect_cap >= mc_range[0] and markect_cap < mc_range[1] and \
        net_in_volume_1m >= net_in_1min_range[0] and net_in_volume_1m < net_in_1min_range[1] and \
            net_in_volume_5m >= net_in_5min_range[0] and net_in_volume_5m < net_in_5min_range[1] and \
                net_in_diff >= net_in_diff_range[0] and net_in_diff < net_in_diff_range[1]

def buy_and_close_judge(parsed_result, wallets_num=4):
    '''判断连续买入，不清仓，连续买入次数大于等于4
    '''
    trade_history = parsed_result['trade_history']
    if trade_history['all_wallets'] >= wallets_num and trade_history['close_wallets'] < 1:
        return True
    return False
...
def token_safe_judge(parsed_result):
    ...
    '''
    token_info = parsed_result['token_info']
    if int(token_info['renounced_mint']) and int(token_info['renounced_freeze_account']) and \
        float(token_info['burn_ratio']) > 0 and str(token_info['burn_status']) == "burn":
        return True
    return False
...
def filter_token_strategy_2(parsed_result, now_time):
    ...
    # kline = parsed_result['kline']
    token_create_time = datetime.strptime(token_info['create_time'], '%Y-%m-%d %H:%M:%S').astimezone(pytz.timezone(time_zone))
    token_open_time = datetime.strptime(token_info['open_time'], '%Y-%m-%d %H:%M:%S').astimezone(pytz.timezone(time_zone))
    launchpad = None
    launchpad_status = 1 # launchpad_status: 0:内盘，1:外盘
    # 钱包数大于等于2，这次信号比上次信号，价格增加了75%
    if_price_increase, price_increase, the_last_buy_price, last_second_buy_price = judge_price_increase(parsed_result)
    
    # 1. 价格上涨满足条件
    if if_price_increase:
        # 1.1 300k以下慢拉性净流入1min净流入300-1k，5min大于3k小于5k
        if quatify_mc_and_net_change(parsed_result, mc_range=(0, 300000),
                                        net_in_1min_range=(300, 1000), net_in_5min_range=(3000, 5000)):
                logger.info(f"token: {token_id} passed regular 1.1 Market cap: {token_info['market_cap']}")
                return {"pass": True, "strategy": "1.1"}
        # 1.2 100k以下快拉性净流入1min大于3k，5min大于9k,并且5min减去1min的差值大于3.5k
        if quatify_mc_and_net_change(parsed_result, mc_range=(0, 100000),
                                        net_in_1min_range=(3000, 1e9), net_in_5min_range=(9000, 1e9), net_in_diff_range=(3500, 1e9)):
            logger.info(f"token: {token_id} passed regular 1.2 Market cap: {token_info['market_cap']}")
            return {"pass": True, "strategy": "1.2"}
        # 1.3 100k以上300k以下，1min净流入2k-6.5k，5min净流入大于6.5k小于15k，或者5min减去1min的差值大于8.5k
        if quatify_mc_and_net_change(parsed_result, mc_range=(100000, 300000),
                                        net_in_1min_range=(2000, 6500), net_in_5min_range=(6500, 15000), net_in_diff_range=(0, 1e9)) or \
                                            quatify_mc_and_net_change(parsed_result, mc_range=(100000, 300000),
                                                                      net_in_1min_range=(0, 1e9), net_in_5min_range=(0, 1e9), net_in_diff_range=(8500, 1e9)):
            logger.info(f"token: {token_id} passed regular 1.3 Market cap: {token_info['market_cap']}")
    ...
    if buy_and_close_judge(parsed_result):
        ...
        if quatify_mc_and_net_change(parsed_result, mc_range=(0, 300000),
                                        net_in_1min_range=(300, 1000), net_in_5min_range=(3000, 5000)):
                logger.info(f"token: {token_id} passed regular 2.1 Market cap: {token_info['market_cap']}")
                return {"pass": True, "strategy": "2.1"}
        # 2.2 100k以下快拉性净流入1min大于3k，5min大于9k,并且5min减去1min的差值大于3.5k
        if quatify_mc_and_net_change(parsed_result, mc_range=(0, 100000),
                                        net_in_1min_range=(3000, 1e9), net_in_5min_range=(9000, 1e9), net_in_diff_range=(3500, 1e9)):
            logger.info(f"token: {token_id} passed regular 2.2 Market cap: {token_info['market_cap']}")
        ...
        if quatify_mc_and_net_change(parsed_result, mc_range=(100000, 300000),
                                        net_in_1min_range=(2000, 6500), net_in_5min_range=(6500, 15000), net_in_diff_range=(0, 1e9)) or \
                                            quatify_mc_and_net_change(parsed_result, mc_range=(100000, 300000),
                                                                      net_in_1min_range=(0, 1e9), net_in_5min_range=(0, 1e9), net_in_diff_range=(8500, 1e9)):
            logger.info(f"token: {token_id} passed regular 2.3 Market cap: {token_info['market_cap']}")
            return {"pass": True, "strategy": "2.3"}
    logger.info(f"Failed to pass regular 2.")
        
    # 3. 当前是第一个信号，且市值大于300k，满足5min-1min净流入大于10k
    if trade_history['all_wallets'] == 1 and token_info['market_cap'] >= 300000 and token_info['net_in_volume_5m'] - token_info['net_in_volume_1m'] > 10000:
        logger.info(f"token: {token_id} passed regular 3. Market cap: {token_info['market_cap']}")
    ...
    if quatify_mc_and_net_change(parsed_result, mc_range=(0, 100000),
                                        net_in_1min_range=(3000, 1e9), net_in_5min_range=(9000, 1e9), net_in_diff_range=(3500, 1e9)):
        logger.info(f"token: {token_id} passed regular 4. Market cap: {token_info['market_cap']}")
    ...
    if trade_history['all_wallets'] == 2:
        ...
        if the_last_buy_price is not None and last_second_buy_price is not None:
            last_second_buy_mc = last_second_buy_price * (token_info['market_cap'] / token_info['price'])
            if last_second_buy_mc > 100000 and price_increase >= 1.75:
                logger.info(f"token: {token_id} passed regular 5. Market cap: {token_info['market_cap']}")
                return {"pass": True, "strategy": "5"}
    logger.info(f"Failed to pass regular 5.")        
    
            
    # 6.信号超过500k，加安全性检测,限制市值在500k-2M
    if token_info['market_cap'] >= 500000 and token_info['market_cap'] <= 2000000 and token_safe_judge(parsed_result) and trade_history['all_wallets'] <= 2:
        # 1. 慢拉性1min<0 5min>7.5k或者0<1min<1k 5min>7.5k
        # 2. 快拉性2k<1min 5min>5k 但是如果5min-1min>13.5k不行
        # 3. 热度盘5min>100k
        result_6_1 = quatify_mc_and_net_change(parsed_result, mc_range=(500000, 2000000),
                                        net_in_1min_range=(0, 1000), net_in_5min_range=(7500, 1e9))
        result_6_2 = quatify_mc_and_net_change(parsed_result, mc_range=(500000, 2000000),
                                        net_in_1min_range=(2000, 1e9), net_in_5min_range=(5000, 1e9), net_in_diff_range=(0, 13500))
        result_6_3 = token_info['net_in_volume_5m'] > 100000
        if result_6_1 or result_6_2 or result_6_3:
            logger.info(f"token: {token_id} passed regular 6. Market cap: {token_info['market_cap']}")
...
def filter_token_strategy_3(parsed_result, now_time):
    ...
    if trade_history['all_wallets'] == 1 and markect_cap >= 500000 and \
        int(token_info['renounced_mint']) and int(token_info['renounced_freeze_account']) and \
            float(token_info['burn_ratio']) > 0 and str(token_info['burn_status']) == "burn":
        logger.info(f"token: {token_id} passed regular 1. Market cap: {markect_cap}")
        return True
    else:
        logger.info(f"token: {token_id} failed to filter strategy 3, part 1. Market cap: {markect_cap}")
    ...
    if trade_history['all_wallets'] == 2 and markect_cap >= 500000 and \
            int(token_info['renounced_mint']) and int(token_info['renounced_freeze_account']) and \
                float(token_info['burn_ratio']) > 0 and str(token_info['burn_status']) == "burn":
        the_last_buy = None
        last_second_buy = None
        ...
        if the_last_buy_price > 0 and last_second_buy_price > 0:
            price_increase = the_last_buy_price / last_second_buy_price
            if price_increase >= 1.8:
                logger.info(f"token: {token_id} passed regular 2. price increase: {price_increase}")
                return True
            else:
                logger.info(f"token: {token_id} failed to filter price increase. price increase: {price_increase}")
                return False
    else:
        logger.info(f"token: {token_id} failed to filter strategy 3, part 2. Market cap: {markect_cap}")
